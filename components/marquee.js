            ...
            <ul className="flex  justify-center md:justify-start [&_li]:mx-4 [&_img]:max-w-none animate-infinite-scroll group-hover:[animation-play-state:paused]">
                {content?.images?.map((image, index) => (
                    <li key={index} {...cslp(content, 'images__', index)}>
                        {!image.image?.url &&
                            <div className="w-[456px] h-[257px] md:w-[616px] md:h-[347px] lg:w-[909px] lg:h-[511px] xl:w-[1203px] xl:h-[676px] bg-gray-400 flex items-center justify-center" {...image?.$?.image}>
                        ...
                        {image?.image?.url &&
                            <a 
                                ...
                                <div className={"flex " + (index % 2 === 0 ? "" : "mt-[52px] lg:mt-[72px] xl:mt-[94px] mb-2")}>
                                    <p className="font-medium text-[40px]/[40px] lg:text-[56px]/[56px] xl:text-[80px]/[80px] tracking-tight max-w-[60%] uppercase" {...image?.$?.title}>{image?.title}</p>
                                    <p className="whitespace-pre-wrap font-work font-medium text-[16px]/[16px]  ml-2 mt-0.5 lg:mt-2 uppercase" {...image?.$?.details}>{image?.details}</p>
                                </div>
                            </a>
                        }
                    </li>

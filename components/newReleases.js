        ...
        <div className="max-w-[720px] md:max-w-[960px] lg:max-w-[1400px] xl:max-w-8xl mx-auto px-2 flex mb-24 md:mb-40">
            <div className="w-[420px] h-[468px] md:w-[560px] md:h-[437px] lg:w-[793px] lg:h-[653px] xl:w-[1049px] xl:h-[869px]  ml-[60px] md:ml-[80px] lg:ml-[117px] xl:ml-[153px] flex flex-col justify-between">
                ...
                {content?.movies?.length === 0 &&
                    <div className="h-[500px] visual-builder__empty-block-parent" {...content?.$?.movies} >
                    </div>
                }
                <div className="uppercase" {...content?.$?.movies}>
                    {content?.movies?.map((item, index) => (
                        <a 
                            href={item?.page?.length > 0 ? item.page[0].url : "#"}
                            key={index} onMouseEnter={() => setCurrentIndex(index)}
                            {...cslp(content, 'movies__', index)}
                        >
                            <p className={"font-oswald font-semibold text-[40px]/[44px] lg:text-[56px]/[66px] xl:text-[80px]/[88px] tracking-tight transition-opacity duration-300 " + (currentIndex === index ? "" : "opacity-20")} {...item.$?.title}>{item.title}</p>
                            <p className={"font-work text-[16px] font-medium " + (currentIndex !== index ? "hidden" : "")} {...item.$?.details}>{item.details}</p>
                        </a>
                    ))}
                </div>
